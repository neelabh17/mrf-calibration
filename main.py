...
def test(config_path, model_path, cuda):
    ...
    import numpy as np
    # tot_conf=np.array([]).reshape(1,-1)
    # tot_obj=np.array([]).reshape(1,-1)
    # tot_label_for_image=np.array([]).reshape(1,-1)
    ...
    for image_ids, images, gt_labels in tqdm(
        ...
        logits=logits/1.2367
        probs = F.softmax(logits, dim=1)
        labels = torch.argmax(probs, dim=1)

        # Neels implementation
        # conf= torch.max(probs.squeeze(0),dim=0)[0].cpu().numpy()
        conf= torch.max(probs.squeeze(0),dim=0)[0].cpu()
        # obj= labels.squeeze(0).cpu().numpy()
        obj= labels.squeeze(0).cpu().float()

        # label_for_image = gt_labels.squeeze(0).view(-1,1).cpu().numpy()
        label_for_image = gt_labels.squeeze(0).view(-1,1).cpu().float()
        # import pdb; pdb.set_trace()

        sel=(label_for_image!=255)

        # tot_conf=np.concatenate((tot_conf,conf.view(-1,1)[sel].view(-1,1)),axis=0)
        # tot_obj=np.concatenate((tot_obj,obj.view(-1,1)[sel].view(-1,1)),axis=0)
        # tot_label_for_image=np.concatenate((tot_label_for_image,label_for_image.view(-1,1)[sel].view(-1,1)),axis=0)
